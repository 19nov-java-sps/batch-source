...
if (!token) {
    window.location.href = "http://localhost:8080/Project_1/mlogin";
} else {
    const tokenArr = token.split(":");
    if (tokenArr.length === 2) {
        const employeeUrl = "http://localhost:8080/Project_1/api/employees";
        const managerUrl = "http://localhost:8080/Project_1/api/managers?username=";
        const reimbursementUrl = "http://localhost:8080/Project_1/api/reimbursements";
        ...
        window.location.href = "http://localhost:8080/Project_1/login";
    }
}
...
function filterReim() {
    ...
    const reimbursementUrl = "http://localhost:8080/Project_1/api/reimbursements?status=Pending&username=";
...
function resolveReim(e) {
    const url = "http://localhost:8080/Project_1/updateReim";
	let xhr = new XMLHttpRequest();
    xhr.open("POST", url);
	xhr.onreadystatechange = function(){
		if(xhr.readyState === 4 && xhr.status === 200){
            window.location.href = "http://localhost:8080/Project_1/managerHome";   // refreshes the page to see new changes.
		}
    }
    const tokenArr = token.split(":");
