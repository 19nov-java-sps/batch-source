...
if (!token) {
    ...
    if (tokenArr.length === 2) {
        const employeeUrl = "http://localhost:8080/Project_1/api/employees?username=";
...
function reqCreate() {
    const url = "http://localhost:8080/Project_1/create";
	let xhr = new XMLHttpRequest();
    xhr.open("POST", url);
	xhr.onreadystatechange = function(){
		if(xhr.readyState === 4 && xhr.status === 200){
            window.location.href = "http://localhost:8080/Project_1/employeeHome";
		}
	}
