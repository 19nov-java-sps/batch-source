...
document.getElementById("approve").addEventListener("click", approveInvoice);
document.getElementById("reject").addEventListener("click", approveReject);
...
if(!token){
	...
	if(tokenArr.length===2){
		let requestUrl = "http://localhost:8080/Project11/api/maninvoices";
		sendAjaxGet(requestUrl, displayInvoices);
	} else {
		window.location.href="http://localhost:8080/Project11/login";
	}
}
...
function approveInvoice(){
	let xhr = new XMLHttpRequest();
	let url = "http://localhost:8080/Project11/pendinvoices";
	xhr.open("POST", url);
	xhr.onreadystatechange = function(){
		if(this.readyState===4 && this.status===200){
	...
	let getInvoiceID = document.getElementById("invoiceID").value;
	let managerID = token[0];
	
	xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded")
	
	let requestBody = `INVOICEID=${getInvoiceID}&MANAGERID=${managerID}`;
...
function approveReject(){
	let xhr = new XMLHttpRequest();
	let url = "http://localhost:8080/Project11/rejectinvoices";
	xhr.open("POST", url);
	xhr.onreadystatechange = function(){
		if(this.readyState===4 && this.status===200){
